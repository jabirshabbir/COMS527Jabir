...
NeuralNetwork* network_create(int input, int output, double lr,int num_stages) {
	...
	net->input = input;
	//initialize the number of outputs of teh hidden 
	...
	{
		//create initialize the hidden layers and randomize the weights.
	...
	//creating buffers to store activations , gradients and hidden errors.
	...
	{
		//creating activations buffers for different stages and for different images in the batch
...
{
	//3 cases. If the pipeline stage =1 . Tak ethe input image
	if (pipeline_stage == 1)
	{
	...
	//if pipeline stage is less than total number of stages then just take the activations 
	...
	//if pipline stage is equal to number of stages.the also produce the outputs 
	else if (pipeline_stage == num_stages)
	{
		Matrix* storedActivation = all_activations[pipeline_stage - 1][input_id];
...
void network_backprop(NeuralNetwork* net,Matrix*output,int pipeline_stage, int input_id,int num_stages)
{
	if (pipeline_stage == num_stages)
	{
		...
		//get the activations from the output stage
		...
		//compute and update gradients for output layer
		all_gradients[pipeline_stage + 1][input_id] = scaled_mat;
		
		//free all temporary tensor.
...
void network_save(NeuralNetwork* net, char* file_string) {
	...
	fprintf(descriptor, "%d\n", net->output);
	fclose(descriptor);
	matrix_save(net->hidden_weights, "hidden1");
